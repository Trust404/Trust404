...
초기 timeout을 20초로 설정한 상태에서 30개 평가를 실행함.

- RISK_001 ~ RISK_029: PASS
- RISK_030: 복합 위험 입력에서 API Timeout 발생
- Timeout 예외 처리가 정상 동작함을 확인

복합 입력 처리 시간을 고려하여 timeout을 45초로 조정하고,
일시적인 API 오류 대응을 위해 최대 1회 재시도를 적용함.

---

### 9/16 안정화 및 최종 회귀 테스트

API 안정성과 반복 실행 시 분류 일관성을 검증함.

주요 안정화 작업:

- OpenAI API timeout 처리 추가
- API 연결 오류, Rate Limit, 상태 코드 오류 처리 추가
- 응답 형식 검증 실패 처리 추가
- timeout을 45초로 조정
- 일시적인 API 오류 대응을 위해 최대 1회 재시도 적용
- 정상 직거래 연락이 `EXTERNAL_CONTACT`로 오탐되는 경계 사례에 후처리 규칙 추가

반복 안정성 테스트:

- 직거래 상황에서 "도착하면 전화 주세요" 등의 연락 표현을 5회 반복 테스트
- 5회 모두 위험 패턴 없음으로 정상 처리
- 실제 "카카오톡으로 이야기하자"는 외부 연락 유도는 `EXTERNAL_CONTACT`로 정상 탐지
...
- RISK_005에서 `PAYMENT_PRESSURE`, `SAFE_PAYMENT_REFUSAL` 외에
  `PREPAYMENT_REQUEST`가 추가 탐지되는 False Positive가 간헐적으로 발생함.
